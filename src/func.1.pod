...
func - fun tools to run port commands in temporary containers
...
Removes the sources and the packages from pkgmk directory, and recreate the F</root> directory, as it might have F<.ccache> or other directories.
